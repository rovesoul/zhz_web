...
class ZHZContracts(admin.ModelAdmin):
    '''设置显示字段,这是自己加的，如果不需要可以在下边register中删掉'''
    list_display = ('name','no','amount','signing_date','p_name','p_no','manager_name','proof','contract_content','first_party','c_time',)
    search_fields = ('name','no','amount','signing_date','p_name','p_no','manager_name','proof','contract_content','first_party','c_time',)  #可搜索字段
    list_editable = ('first_party','manager_name','proof',) #默认可编辑字段
    ordering = ('no','signing_date')
    list_filter = ('p_name', 'first_party','manager_name','proof',)  # 过滤器
